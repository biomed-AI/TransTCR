...
class Entropic_OT_Loss(nn.Module):
    ...
    def entropic_ot(self, M, reg=0.01, n=5):
        ...
        a = torch.ones(M.shape[0], device=self.device)
        b = torch.ones(M.shape[0], device=self.device) / M.shape[0]

        # initialize u and v as uniform distributions
        u = torch.ones(a.shape[0],device=self.device) / a.shape[0]
        v = torch.ones(b.shape[0],device=self.device) / b.shape[0]
    ...
    def forward(self, all_image_features, all_text_features, labels=None):
        logits_per_image, logits_per_text = self.get_logits(all_image_features, all_text_features)
        
        # print(self.entropic_ot(1.0 - logits_per_image),self.entropic_ot(1.0 - logits_per_text))
        
        return self.entropic_ot(1.0 - logits_per_image),self.entropic_ot(1.0 - logits_per_text)
        # loss = (
...
class DBOT_Sinkhorn_Loss(nn.Module):
    ...
    def forward(self, all_image_features, all_text_features, labels=None):
        ...
        return self.dbot_sinkhorn(1.0 - logits_per_image),self.dbot_sinkhorn(1.0 - logits_per_text)
        # loss = (
